...
```{r}
f <- "/Users/julienbryios/Documents/Data/Projects/Best_tissue2/Data/GTEx/GTEx_Analysis_2016-01-15_v7_RNASeQCv1.1.8_gene_median_tpm.gct"
```

```{r}
exp <- read_tsv(f,skip=2)
```

```{R, load data 3,message=FALSE, warning=FALSE}
exp_lvl5 <- gather(exp,key = Lvl5,value=Expr_sum_mean_raw,-gene_id,-Description) %>% as_tibble() %>% rename(Gene="Description")
```
...
exp_lvl5 %>% write_tsv(gzfile("../../02_Processed//GTEx.v7.all.norm.txt.gz"))
```