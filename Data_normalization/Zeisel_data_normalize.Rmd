...
```{r}
all.equal(sumstats_final$total_sum_umi_scaled10k,rep(10000,265))
```
...
```{r}
exp_lvl5 <- exp_lvl5 %>% mutate(Lvl5=paste0(Description," (",Lvl5,")"))
exp_lvl5 <-  select(exp_lvl5,Gene,gene_id,entrez_id,Lvl5,spe_10k) %>% spread(Lvl5,spe_10k)
exp_lvl5 %>% write_tsv(gzfile("../../02_Processed/Zeisel.1to1.norm.txt.gz"))
```