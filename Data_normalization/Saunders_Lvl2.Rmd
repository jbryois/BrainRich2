...
```{r}
exp_lvl5 <- exp_lvl5 %>% group_by(tissue_class,Gene) %>% summarise(Expr_sum_mean=sum(Expr_sum_mean)) %>% ungroup()
```
...
```{r}
all.equal(sumstats_final$total_sum_umi_scaled10k,rep(10000,88))
```
...
```{r}
dic <- read_tsv("~/Documents/Data/Projects/Best_tissue2/Data/Saunders/dictionary_cell_type_names.lvl2.txt")
exp_lvl5 <- inner_join(exp_lvl5,dic,by=c("Lvl5"="tissue_class")) %>% select(-Lvl5) %>% rename(Lvl5=NiceName)
```
...
exp_lvl5 %>% write_tsv(gzfile("../Shiny/BrainRich2/Data/Saunders.lvl2.1to1.norm.txt.gz"))
```