...
```{r}
cell_types_lvl3 <- select(cell_types,Lvl1:Lvl3) %>% unique()
```

```{r}
exp_lvl5 <- exp_lvl5 %>% group_by(Lvl3,Gene) %>% summarise(Expr_sum_mean=mean(Expr_sum_mean))
exp_lvl5 <- inner_join(exp_lvl5,cell_types_lvl3)
```

```{r}
exp_lvl5 <- exp_lvl5 %>% group_by(Lvl2,Gene) %>% summarise(Expr_sum_mean=mean(Expr_sum_mean)) %>% rename(Lvl5=Lvl2)
```
...
exp_lvl5 %>% write_tsv(gzfile("../Shiny/BrainRich2/Data/Zeisel.lvl2.1to1.norm.txt.gz"))
```