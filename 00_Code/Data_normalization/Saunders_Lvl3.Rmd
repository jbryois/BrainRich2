...
```{r}
exp_lvl5 <- exp_lvl5 %>% group_by(common_name,Gene) %>% summarise(Expr_sum_mean=sum(Expr_sum_mean)) %>% ungroup()
```

```{r}
exp_lvl5 <- rename(exp_lvl5,Lvl5=common_name)
```
...
exp_lvl5 %>% write_tsv(gzfile("../Shiny/BrainRich2/Data/Saunders.lvl3.1to1.norm.txt.gz"))
```