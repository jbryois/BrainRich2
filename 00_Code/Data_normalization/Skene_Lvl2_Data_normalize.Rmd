...
```{r}
f <- "/Users/julienbryios/Documents/Data/Projects/Best_tissue2/Data/KI_lvl2/celltype_data_allKImouse_wtHypo_MergedStriatal_1to1only_level2_thresh0_trim0.rda"
```
...
exp_lvl5 <- gather(exp,key = Lvl5,value=Expr_sum_mean,-Gene) %>% as.tibble()
```
...
```{r}
all.equal(sumstats_final$total_sum_umi_scaled10k,rep(10000,149))
```
...
```{r}
exp_lvl5 <- inner_join(exp_lvl5,entrez_ensembl,by="entrez_id")
```
...
exp_lvl5 %>% write_tsv(gzfile("../../02_Processed//Skene_lvl2.1to1.norm.txt.gz"))
```
